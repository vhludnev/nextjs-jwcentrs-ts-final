...
const TerritoryMainPopup = dynamic(
  () => import("@/components/popups/TerritoryMainPopup")
);
...
const TableHead = ({ handleSorting, sortField, sortOrder }: Props) => {
    ...
    <thead className="text-teal border dark:filter dark:brightness-75 dark:border-gray-400">
      <tr className="bg-gray-200 table-row rounded-l-lg rounded-none mb-4 md:mb-0 text-center dark:text-gray-900">
        {COLUMNS.map(({ label, accessor, sortable }) => {
          ...
          const view = ["publisher", "given"].includes(accessor)
            ? "hidden sm:visible sm:table-cell"
            : "";
          ...
          const availb =
            accessor === "available"
              ? "text-transparent text-[4px] md:text-base md:text-inherit"
              : "";
          return (
            <th
  ...
  return (
    <tbody className="flex-none dark:filter dark:brightness-90">
      {tableData.map((data, idx) => {
        const expired =
          data.given && differenceInMonths(new Date(), data.given) > 4;
        const urgentToTake =
          data.returned && differenceInMonths(new Date(), data.returned) > 12;
        ...
        return (
          <tr
            className={`table-row mb-4 md:mb-0 dark:text-gray-800 ${expiredOrUrgentStyle}`}
            key={data.id}
          >
            {COLUMNS.map(({ accessor }) => {
              ...
              switch (accessor) {
                ...
                case "returned":
                  tData = data[accessor]
                    ? format(data[accessor], "dd.MM.yyyy")
                    : "";
                  break;
                default:
                  tData = data[accessor];
...
const Territories = () => {
  ...
  const changePage = (value: number) =>
    setPage(value); /* (value < 1 ? setPage(1) : setPage(value)) */
    ...
    return (
      <>
        <h1 className="head_text mt-0 md:mt-2 text-center">
          <span className="blue_gradient">Обрабатываемые территории</span>
        </h1>
  ...
  return (
    <>
      <h1 className="head_text blue_gradient mt-0 md:mt-2 text-center">
        Обрабатываемые территории
      </h1>
      <div className="flex items-center justify-between mt-6 md:mt-10 md:px-2 gap-2 sm:gap-4">
        ...
        <SearchComponent handleSearch={handleSearch} criteria={criteria} />

        <NewTerritoryPopup createNewTerritory={handleCreateNewTerritory} />
      </div>

      {!fetchingData ? (
        <>
          <table className="w-full rounded-lg overflow-hidden md:shadow-lg my-5 text-sm md:text-base md:mx-auto">
            <TableHead {...{ handleSorting, sortField, sortOrder }} />
            <TableBody
              {...{
                tableData:
                  newData?.territories ??
                  [] /* filteredAndSortedData?.territories */,
                setPopupData,
                setMainOpen,
                page,
                count,
          ...
          {newData &&
            typeof newData.totalPages === "number" &&
