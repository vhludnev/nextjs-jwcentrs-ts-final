...
const SearchBox = forwardRef<Ref, Props>(({ onChange, criteria, /* placeholder, */ onSubmit = () => {} }, ref) => {
  ...
  return (
    <form className='relative dark:filter dark:brightness-90' onSubmit={onSubmit}>
      <input
        ref={ref}
        type='search'
        ...
        className={`bg-transparent peer cursor-pointer border-0 relative z-10 h-10 md:h-12 w-12 rounded-full outline-none transition-all duration-500 transform ${onFocusStyles} ${
          criteria ? onCriteria : 'text-transparent caret-transparent placeholder:text-transparent'
        }`}
      />
      <SearchIcon
