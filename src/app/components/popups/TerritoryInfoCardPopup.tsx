...
import { MdOutlineMapsHomeWork, AiOutlineCloudUpload, TbZoomReset, BiZoomIn, BiZoomOut } from '@/lib/icons'
import Modal from './Modal'
import Spinner from '../Spinner'
import { handleAddImage, handleDeleteImage } from '@/lib/cloudinary'
import { useTerritoryInfoCard } from '@/lib/territories'
import { cc, /* convertUrlToBase64, convertFileToBase64, */ placeholder, rounded } from '@/utils'
...
const Controls = () => {
  ...
  return (
    <div className='flex gap-2 md:gap-3 absolute left-2 top-1 md:top-2 z-50 opacity-60 bg-primary-white p-1 rounded-md'>
      <BiZoomIn cursor='pointer' className='text-xl md:text-2xl text-gray-600' onClick={() => zoomIn()} />
      <BiZoomOut cursor='pointer' className='text-xl md:text-2xl text-gray-600' onClick={() => zoomOut()} />
      <TbZoomReset cursor='pointer' className='text-xl md:text-2xl text-gray-600' onClick={() => resetTransform()} />
    </div>
  )
}
...
const TerritoryInfoCardPopup = ({
  ...
  const onInputChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const fileMimeTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/bmp', 'image/webp']
  ...
  return (
    ...
    >
      ...
      <div
        className={cc('content', rounded('b', 'lg'), 'px-0 md:px-2 py-2 bg-primary-white dark:bg-[#454b4d] -mt-0.5')}
      >
        <div className='text-base relative pb-7 md:overflow-auto w-full md:min-w-[600px] flex flex-col items-center'>
          ...
          <div className='w-full relative mb-6'>
            {imagePreview && !loadingTerrInfoCard ? (
              ...
              <div className='h-[193px] md:h-[393px] inline-block'>{/* <Spinner /> */}</div>
            )}
          ...
          {editable ? (
            <>
              <form>
                <input type='file' accept='image/*' onChange={onInputChange} ref={hiddenFileInput} className='hidden' />
              </form>
              <div className='w-[90%] text-center'>
                ...
                <textarea
                  ...
                  onBlur={() => address && handleUpdateTerritoryAddress({ id, address })}
                />
              </div>
            </>
          ) : (
            <div className='dark:text-gray-100 px-2 w-full text-center'>
              <span>{t('location')}:</span>
              <br />
              <b className='whitespace-break-spaces'>{address ?? t('no-info')}</b>
            </div>
          )}
          {terrInfoCardData?.comment && (
            <div className='dark:text-gray-100 pt-4 px-2 w-full text-center'>
              <span>{t('notes')}:</span>
              <br />
              <b className='whitespace-break-spaces'>{terrInfoCardData.comment}</b>
            </div>
          )}
        </div>
      </div>
